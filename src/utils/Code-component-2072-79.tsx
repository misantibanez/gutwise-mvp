...
class GamificationService {
  ...
  getStats(): GamificationStats {
    ...
    return {
      totalMealsLogged: 0,
      totalSymptomsTracked: 0,
      currentStreak: 0,
      longestStreak: 0,
      lastActivityDate: null,
      weeklyGoalMeals: 7, // Goal: 1 meal per day
      weeklyGoalSymptoms: 3, // Goal: Track symptoms when they occur
      level: 1,
      experiencePoints: 0,
      achievements: [],
      weeklyProgress: {
  ...
  generateDoctorReport(stats: GamificationStats): string {
    const weeklyProgress = this.getWeeklyProgress(stats);
    const today = new Date().toLocaleDateString();
    
    return `
GutWise Progress Report
Generated: ${today}
...
📊 Overall Statistics:
• Total Meals Logged: ${stats.totalMealsLogged}
• Total Symptoms Tracked: ${stats.totalSymptomsTracked}
• Current Activity Streak: ${stats.currentStreak} days
• Longest Streak: ${stats.longestStreak} days
• User Level: ${stats.level} (${stats.experiencePoints} XP)

📅 This Week's Activity:
• Meals Logged: ${stats.weeklyProgress.mealsThisWeek}/${stats.weeklyGoalMeals} (${Math.round(weeklyProgress.mealsProgress)}%)
• Symptoms Tracked: ${stats.weeklyProgress.symptomsThisWeek}/${stats.weeklyGoalSymptoms} (${Math.round(weeklyProgress.symptomsProgress)}%)
• Weekly Goal Status: ${weeklyProgress.isWeeklyGoalMet ? '✅ Met' : '⏳ In Progress'}

🏆 Achievements Unlocked (${stats.achievements.length}):
${stats.achievements.slice(0, 5).map(achievement => `• ${achievement.name}: ${achievement.description}`).join('\n')}
${stats.achievements.length > 5 ? `• ...and ${stats.achievements.length - 5} more` : ''}

📈 Engagement Level: ${this.getEngagementLevel(stats)}

NOTES FOR HEALTHCARE PROVIDER:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
This report shows the patient's engagement with digital health tracking. High engagement typically correlates with better treatment adherence and health outcomes.

- Consistent daily logging indicates good habit formation
- Symptom tracking frequency shows awareness of health patterns
- Achievement system encourages continued engagement
- Streak maintenance demonstrates commitment to health monitoring

Generated by GutWise Digital Health Platform
    `.trim();
  }
